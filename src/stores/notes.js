...
export const useNotesStore = defineStore('notes', () => {
  ...
  async function createNote(title, content) {
    const res = await axios.post('http://localhost:5000/api/notes', { title, content }, { headers: getHeaders() })
  ...
  async function updateNote(id, title, content) {
    const res = await axios.put(`http://localhost:5000/api/notes/${id}`, { title, content }, { headers: getHeaders() })
    const index = notes.value.findIndex(n => n._id === id)
    if (index !== -1) notes.value[index] = res.data
  }

  return { notes, loading, fetchNotes, createNote, deleteNote, updateNote }
})
